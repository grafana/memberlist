...
//
// The claimed decoded length is checked against maxDecompressedBytes before
...
func snappyDecompress(src []byte) ([]byte, error) {
	...
	if n > maxDecompressedBytes {
		return nil, fmt.Errorf("memberlist: snappy-decompressed payload would exceed %d bytes (claimed %d)", maxDecompressedBytes, n)
	}
